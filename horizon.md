...
> [!WARNING]
> You should ensure that the `environments` portion of your `horizon` configuration file contains an entry for each [environment](configuration.md#configuracao-do-ambiente) on which you plan to run Horizon.
...
While your application is in [maintenance mode](configuration.md#modo-de-manutencao), queued jobs will not be processed by Horizon unless the supervisor's `force` option is defined as `true` within the Horizon configuration file:
